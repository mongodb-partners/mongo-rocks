...
namespace mongo {
    ...
    public:
        ...
        std::unique_ptr<RecordStore> newCappedRecordStore(const std::string& ns,
            ...
            return stdx::make_unique<RocksRecordStore>(
                ns, "1", _db.get(), _counterManager.get(), _durabilityManager.get(),
                _compactionScheduler.get(), "prefix", true, cappedMaxSize, cappedMaxDocs);
        }
    ...
    RecordId _oplogOrderInsertOplog(OperationContext* opCtx, std::unique_ptr<RecordStore>& rs,
                                    int inc) {
